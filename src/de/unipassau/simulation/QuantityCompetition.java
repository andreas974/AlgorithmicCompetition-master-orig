...
class QuantityCompetition extends Competition {
    ...
    public double calculateProfit(double quantity, double price, int numberOfFirms) {
        return (quantity * quantityScalingCoefficient) * price * profitScalingCoefficient;
    }
     ...
     */
    @Override
    public double getScaledProfitInNashEquilibrium(int numberOfFirms) {
        return profitScalingCoefficient * (Math.pow(omega, 2)) / (lambda * Math.pow(2 + theta * (numberOfFirms - 1), 2));
    }
     ...
     */
    @Override
    public double getScaledProfitInJpmEquilibrium(int numberOfFirms) {
        return profitScalingCoefficient * (Math.pow(omega, 2)) / (4 * lambda * (1 + theta * (numberOfFirms - 1)));
    }
